<section>
    <header class="mb-8">
        ...
        <p class="text-sm text-slate-500 leading-relaxed">
            {{ __('Gunakan kombinasi simbol, angka, dan huruf kapital untuk menjaga keamanan akun Anda.') }}
        </p>
    </header>
    ...
    <form method="post" action="{{ route('password.update') }}" class="space-y-5">
        ...
        <div class="flex items-center gap-4 pt-2">
            ...
            @if (session('status') === 'password-updated')
                <div x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)" class="flex items-center gap-2 text-emerald-600">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>
                    <p class="text-sm font-bold">{{ __('Tersimpan!') }}</p>
                </div>
            @endif
        </div>
    </form>
